...
const FinAbbrPage = () => {
  ...
  const [embeddingOptions, setEmbeddingOptions] = useState({
    ...
    collectionName: 'concepts_fin_only_name'
  });
  ...
  const handleSubmit = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/finabbr`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <h1 className="text-3xl font-bold mb-6">金融术语缩写展开 📝</h1>
      
      <div className="grid grid-cols-3 gap-6">
        {/* Left panel: Text inputs */}
        <div className="col-span-2 bg-white shadow-md rounded-lg p-6">
          <h2 className="text-xl font-semibold mb-4">输入金融信息</h2>
          <TextInput
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="请输入包含缩写的金融信息..."
          />
