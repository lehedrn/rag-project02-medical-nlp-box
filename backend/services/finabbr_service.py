...
class FinAbbrService:
    ...
    def _get_std_service(self, embedding_options: dict) -> FinStdService:
        """
        获取或创建标准化服务实例
        ...
        """
        try:
            return FinStdService(
                provider=embedding_options.get("provider", "huggingface"),
                model=embedding_options.get("model", "BAAI/bge-m3"),
                db_path=f"db/{embedding_options.get('dbName', 'finterm_bge_m3.db')}.db",
                collection_name=embedding_options.get("collectionName", "concepts_fin_only_name")
            )
        except Exception as e:
            logger.error(f"Failed to initialize FinStdService: {str(e)}")
            raise ValueError(f"Failed to initialize financial standardization service: {str(e)}")
    ...
    def simple_ollama_expansion(self, text: str, llm_options: dict) -> Dict:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "Your job is to return the input with ALL abbreviations in the financial domain replaced with their expanded forms."),
            ("system", "Input consists of financial texts. Keep all occurrences of ___ in the output."),
