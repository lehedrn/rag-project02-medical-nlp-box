...
# 文件路径
file_path = "backend/data/finterm.csv"
db_path = "backend/db/finterm_bge_m3.db"

# 连接到 Milvus
client = MilvusClient(db_path)

collection_name = "concepts_fin_only_name"
...
logging.info(f"Search result for 'CPI': {search_result}")

# 查询所有匹配的实体
query_result = client.query(
    collection_name=collection_name,
    filter="concept_name == 'ETF'",
    output_fields=["concept_name"],
    limit=5
)
logging.info(f"Query result for concept_name == 'ETF': {query_result}")
